...
// Total number of orders for this staff (count only unique order IDs)
$query = "SELECT COUNT(DISTINCT order_id) AS total_orders FROM rpos_orders WHERE staff_id = ?";
$stmt = $mysqli->prepare($query);
...
$query = "SELECT IFNULL(SUM(total_price), 0) AS total_sales FROM rpos_orders WHERE staff_id = ?";
$stmt = $mysqli->prepare($query);
$stmt->bind_param('i', $staff_id);
$stmt->execute();
$res = $stmt->get_result();
...
<div class="main-content">
      ...
      <div class="header-body">
        <div class="row">
          <div class="col-xl-3 col-lg-6">
            <div class="card card-stats mb-4 mb-xl-0" style="background-color: #F7E6CA;">
              <div class="card-body">
                <div class="row">
                  <div class="col">
  ...
  <div class="col-xl-12 mb-5 mb-xl-0">
    <div class="card shadow" style="background-color: #F7E6CA; border-radius: 10px; padding: 15px;">
...
<script>
  ...
  function loadSalesData(filter = 'this_month') {
    fetch(`fetch_sales_data.php?filter=${filter}&staff_id=${staffId}`)
      .then(res => res.json())
      .then(data => {
        if (salesChart) {
