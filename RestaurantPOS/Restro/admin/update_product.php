...
if (isset($_POST['UpdateProduct'])) {
  ...
  if (empty($_POST["barcode"]) || empty($_POST["prod_name"]) || empty($_POST['prod_desc']) || empty($_POST['prod_price'])) {
    ...
    $postQuery = "UPDATE rpos_products SET barcode =?, prod_name =?, prod_img =?, prod_desc =?, prod_price =? WHERE prod_id = ?";
    $postStmt = $mysqli->prepare($postQuery);
    //bind paramaters
    $rc = $postStmt->bind_param('ssssss', $barcode, $prod_name, $prod_img, $prod_desc, $prod_price, $update);
    $postStmt->execute();
              ...
              <div class="card-body">
                <form method="POST" enctype="multipart/form-data">
                  <div class="form-row">
                    ...
                    <div class="col-md-6">
                      <label>Barcode</label>
                      <input type="text" name="barcode" value="<?php echo $prod->barcode; ?>" class="form-control" value="">
                    </div>
                  </div>
                  <hr>
                  <div class="form-row">
