...
if (isset($_GET['book'])) {
    ...
    $pass_id = intval($_GET['book']);

    // Check pass availability
    $pass_query = mysqli_query($conn, "SELECT * FROM passes WHERE id = $pass_id");
    $pass = mysqli_fetch_assoc($pass_query);

    if ($pass) {
        $max_capacity = $pass['max_capacity'];
        $current_bookings = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as count FROM reservations WHERE pass_id = $pass_id AND status != 'Cancelled'"))['count'];
        ...
        if ($current_bookings < $max_capacity) {
            ...
            mysqli_query($conn, "INSERT INTO reservations (user_id, pass_id, reservation_date, reservation_time, status) 
                                 VALUES ('$user_id', '$pass_id', '$today', '$time', 'Pending')");
            $message[] = 'Reservation submitted! Please wait for admin confirmation.';
        } else {
            $message[] = 'Sorry, this pass is fully booked.';
        }
    }
}

$passes = mysqli_query($conn, "SELECT * FROM passes");
?>
